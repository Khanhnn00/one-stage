...
def load_img_name_list(dataset_path, index='.jpg'):
    # print(dataset_path)
    img_gt_name_list = open(dataset_path).read().splitlines()
...
def load_label_name_list(dataset_path):
    return load_img_name_list(dataset_path, '.jpg')
...
class VOC12ClsDataset(VOC12ImageDataset):
    ...
    def __getitem__(self, idx):
        ...
        mask = Image.new("P", (100, 100), color=20) # pallete (indexed)
        mask.putpixel((0, 0), 1) # 1-pixel 'person' mask! # you need this!
        mask = np.array(mask)
